...
public class AssessmentPart1 {
    ...
    */
    public int Q04_moviePriceMatinee(int age, boolean isBefore5pm, boolean isWeekend) {
        int generalAdmissionMatineePrice = 8;
        int childrenMatineePrice = 5;
        int generalAdmissionNonMatineePrice = 12;
        int childrenNonMatineePrice = 7;

        boolean isMatinee = isBefore5pm && !isWeekend;

        if (isMatinee) {
            if (age <= 12) {
                return childrenMatineePrice;
            } else {
                return generalAdmissionMatineePrice;
            }
        } else {
            if (age <= 12) {
                return childrenNonMatineePrice;
            } else {
                return generalAdmissionNonMatineePrice;
            }
        }
    }
    ...
    */
    public Map<String, Integer> Q11_mealCount(String[] mealOrders) {
        Map<String, Integer> mealNumberOfTimes = new HashMap<>();
        for (String meal : mealOrders) {
            if (mealNumberOfTimes.containsKey(meal)) {
                mealNumberOfTimes.put(meal, mealNumberOfTimes.get(meal) + 1);
            } else {
                mealNumberOfTimes.put(meal, 1);
            }
        }
        return mealNumberOfTimes;
    }
}
