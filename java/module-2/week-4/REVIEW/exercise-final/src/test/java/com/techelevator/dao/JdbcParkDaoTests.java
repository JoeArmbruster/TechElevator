...
public class JdbcParkDaoTests extends BaseDaoTests {
    ...
    public void getParkById_returns_correct_park_for_id() {
        ...
        Park park = jdbcParkDao.getParkById(1);
        Assert.assertNotNull("getParkById(1) returned null", park);
        assertParksMatch("getParkById(1) returned wrong or partial data", PARK_1, park);

        park = jdbcParkDao.getParkById(2);
        Assert.assertNotNull("getParkById(2) returned null", park);
        assertParksMatch("getParkById(2) returned wrong or partial data", PARK_2, park);

        park = jdbcParkDao.getParkById(3);
        Assert.assertNotNull("getParkById(3) returned null", park);
        assertParksMatch("getParkById(3) returned wrong or partial data", PARK_3, park);

        // Park 4 does not exist
        park = jdbcParkDao.getParkById(4);
        Assert.assertNull("getParkById(4) does not exist and should be null", park);
    }

    @Test
    public void getParks_returns_all_parks() {

        List<Park> parks = jdbcParkDao.getParks();
        Assert.assertEquals("getParks() returned wrong number of parks", 3, parks.size());
    }
